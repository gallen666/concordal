...
def fetch_sonar_news(
    ...
    *,
    ...
    # Sonar doesn't expose per-headline publication dates in its current
    # API shape — only the answer text + a flat list of citation URLs.
    # Use "now" as published_at: these are all 'today' news by construction
    # (our prompt asks for today's news) and the news analyst only cares
    # about the date for display + chronological ordering.
