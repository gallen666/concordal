...
class CnEquityAdapter(MarketAdapter):
    ...
    def get_fundamentals(self, ticker: str, asof: date) -> Fundamentals:
        ...
        if not self._available:
            # NEVER fabricate fundamentals via MockAdapter — LLM analysts
            # have no way to tell mock numbers from real ones, and that's
            # exactly how hallucinated "TTM revenue 2220 亿" ends up in the
            # report. Return empty Fundamentals with explicit notes; the
            # analyst prompts treat empty fields as "unavailable" and are
            # instructed not to imagine numbers.
            return Fundamentals(
                ticker=ticker, asof=asof,
                notes=(
                    "akshare not installed on this server. "
                    "FUNDAMENTALS UNAVAILABLE — DO NOT FABRICATE NUMBERS. "
                    "Base analysis only on price action and macro context."
                ),
            )
        try:
            ak = self._ak()
            ...
            log.warning("akshare fundamentals failed for %s: %s", ticker, e)
            # SAME RULE as the not-installed path: never fabricate via mock.
            # Try multi-source for at least a name + market cap (no revenue
            # / margin data available from these sources). Falling back to
            # mock here was the root cause of the "2220 亿 TTM revenue"
            # hallucination — a number the LLM trusted as factual but was
            # generated by MockAdapter from a deterministic seed.
            try:
                from .cn_stock_multi_source import fetch_a_share_quote_multi
                q = fetch_a_share_quote_multi(ticker)
            except Exception:
            ...
            return Fundamentals(
                ticker=ticker, asof=asof,
                notes=(
                    f"akshare 基本面拉取失败 ({type(e).__name__})。"
                    + (f"已从 {q.get('source')} 拿到公司名 '{q.get('name')}' 作为唯一可靠字段。" if q else "")
                    + " FUNDAMENTALS UNAVAILABLE — 不要编造营收/利润率/市值 数字。"
                    + " Base analysis on price action + macro + sentiment only."
                ),
            )
